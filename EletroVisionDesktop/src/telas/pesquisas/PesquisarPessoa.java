...
import telas.comuns.tablemodel.PessoaTableModel;
...
public class PesquisarPessoa extends javax.swing.JPanel {
    ...
    public PesquisarPessoa() {
        initComponents();
        InstanciasTelas.setPesquisarPessoa(PesquisarPessoa.this);
    ...
    private void initComponents() {
        ...
        cb_chave.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Código", "Nome", "CPF", "Nascimento", "Tipo" }));
        ...
        jPanel2Layout.setHorizontalGroup(
            ...
            .addGroup(jPanel2Layout.createSequentialGroup()
                ...
                .addComponent(tf_pesquisa, javax.swing.GroupLayout.DEFAULT_SIZE, 366, Short.MAX_VALUE)
                .addContainerGap())
        );
...
private void tf_pesquisaKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tf_pesquisaKeyReleased
        ...
        if (verificarLinhaSelecionada()) {
            ...
            if (InstanciasControle.getMensagens().confirmacao(MensagensSoltas.pesqComumExcluir())) {
                ...
                for (int i = 0; i < linhas.length; i++) {
                    pessoa = (PessoaBean) tb_pesquisa.getValueAt(linhas[i], -1);
                    pessoa = excluirPessoa(pessoa);
                }
                InstanciasControle.getGerenciarAudio().exeDeletar();
                if (pessoa != null) {
                    InstanciasControle.getMensagens().sucesso(MensagensSoltas.pesqPessoaExcluir());
                }
                carregarTabela();
            }
        }
    ...
    public void editar() {
        if (verificarLinhaSelecionada()) {
            ...
            SwingUtil.adicionarTabComBotaoFechar(InstanciasTelas.getPrincipal().tb_principal, "Editar Pessoa", cadastro);
            InstanciasTelas.getPrincipal().esperarTelas();
            CadastrarPessoa pessoa = new CadastrarPessoa();
            pessoa.preencheCampos((PessoaBean) tb_pesquisa.getValueAt(tb_pesquisa.getSelectedRow(), -1));
            InstanciasTelas.getPrincipal().redimencionarPainelCadastro(pessoa, cadastro);
            cadastro.pn_principal.add(pessoa);
            cadastro.verificaInstancias(pessoa);
        }
    }

    private void popularTabela(ArrayList<PessoaBean> lista) {
        PessoaTableModel modelo = new PessoaTableModel(lista);
    ...
    private void carregarTabela() {
        popularTabela(listarPessoaOrdenadaPorCodigo());
    }
    ...
    private boolean verificarLinhaSelecionada() {
        ...
        if (linhaSelecionada != -1) {
            ...
            InstanciasControle.getMensagens().bug(MensagensSoltas.pesqPessoaSelecionar());
            return false;
        }
    }
    ...
    public void ajuda() {
        Manual manual = new Manual(ArquivosHTML.pesquisas()+ArquivosHTML.pessoa());
    ...
    private PessoaBean excluirPessoa(PessoaBean pessoa) {
        PessoaFacade pessoaFacade = FactoryFacade.getPessoaImpl();
        try {
            pessoa.setDeletado(EnumDeletadoPessoa.SIM);
            return pessoaFacade.alterar(pessoa);
        } catch (FacadeException ex) {
        }
        return null;
    }
