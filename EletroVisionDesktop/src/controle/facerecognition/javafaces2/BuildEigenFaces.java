package controle.facerecognition.javafaces2;
...
/*
 * Use the training face images in trainingImages\ to create eigenvector and
 * eigenvalue, information using PCA. The information is stored in eigen.cache.
 * Two subdirectories, eigenfaces\ and reconstructed\, are also generated which
 * contain eigenfaces and regenerated images. These are only produced to allow
 * the eigenface process to be checked.  *
 * Only eigen.ccache is used by the recognition process, which is separated out
 * into FaceRecognizer.java
 *
 * This code is a refactoring of the JavaFaces package by Sajan Joseph,
 * available at http://code.google.com/p/javafaces/ The current version includes
 * a GUI.
 */
...
public class BuildEigenFaces {
     ...
     * calculate weights x eigenfaces, which generates mean-normalized traimning
